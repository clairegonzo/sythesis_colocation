...
ggplot(data = total_df_tidy, aes(x = activity, y = counts)) +
  ...
  geom_text(aes("Aquaculture", 102,label = "n=99", hjust= .5), size = 3) + # n values from sums df
  geom_text(aes("Tidal", 60,label = "n=57", hjust= .5), size = 3) +
  geom_text(aes("Wave", 112,label = "n=109", hjust= .5), size = 3) +
  geom_text(aes("Wind", 149,label = "n=146", hjust= .5), size = 3) +
  geom_text(aes("Other", 84,label = "n=81", hjust= .5), size = 3)

# colorful pallete is "calecopal::bigsur"
```
...
ggsave(filename = here("figures", "fig2_new.jpeg"), width = 7, height = 5, units = "in")
```
...
ggplot(data = total_df_tidy, aes(x = activity, y = counts)) +
  ...
  #theme(axis.text.x = element_text(angle = 30)) +
  geom_text(aes("Aquaculture",1.03,label = "n=99", hjust= .5), size = 3) + # n values from sums df
  geom_text(aes("Tidal",1.03,label = "n=57", hjust= .5), size = 3) +
  geom_text(aes("Wave",1.03,label = "n=109", hjust= .5), size = 3) +
  geom_text(aes("Wind",1.03,label = "n=146", hjust= .5), size = 3) +
  geom_text(aes("Other",1.03,label = "n=81", hjust= .5), size = 3)
```
...
# "Here Is an example of plot where I did this! It was tricky but you just have to redefine the column 
# in the df with the nested `factors()` and `levels()` functions, then you can keep the code for the ggplot the same, just defining the x axis as the column name as you did before" 
